				...
				<ul>
					{
						activityList && activityList.map((item, index) => {
							return (
								<li className='pdt_item' key={index} onClick={this.toUrl.bind(this, `/activityDetails?from=carModelDetail&activityId=${item.id}`)}>
									<div className='pdt_cover'>
										<img src={item.image} />
									</div>

									<div className='pdt_infoBox'>
										<div className='pdt_info1 ellipsis-three'>{item.name}</div>
