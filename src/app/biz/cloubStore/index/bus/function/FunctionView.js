...
class CFunction extends Component{
    ...
    //快捷栏点击
    handleClickLink = (redirectUrl) => {
        let sp = '?' ;
        if(redirectUrl.indexOf("?")!=-1){
            sp = "&" ;
        }
        IndexUtil.toUrl(`${redirectUrl}${sp}dealerId=${this.stores.state.dealerId}&from=cloubStore/index&tabBarType=1`) ;
    }
    render(){
                    ...
                    <div className='cfunction-nav'>
                        <Flex className="nav">
                            ...
                            { quickLinkList.map((item, index) => {
                                ...
                                if(index>=4) return '';
                                return (
                                    <div className="nav-item" onClick={() => { this.handleClickLink(item.redirectUrl) }} key={'nav'+index}>
                                        <img src={item.imgUrl} />
                                        <span>{item.name}</span>
                                    </div>
                                )
