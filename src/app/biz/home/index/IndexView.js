	...
	getCurrCity() {
		...
		let callbacks = {
			complete: function (results) {
				...
				self.stores.setPosition({ label: cityName, value: postCode, provinceCode: provinceCode })
			}
		}
							...
							<Carousel
								...
								swipeSpeed={35}>
								{
									bannerList.map((item, index) => {
										return (
											<div className="banner-item-wrap" key={'banner' + index} onClick={this.logoBannerToUrl.bind(this, item)}>
				...
				<div className="nav-wrap">
								...
								newsList.length > 0 ? (
									...
									>
										{newsList.map((item, index) => {
											return (
												<div className="news-item ellipsis" key={'news' + index} onClick={this.gotoUrl.bind(this, item.redirectUrl)}>
					...
					<Flex className="nav" wrap="wrap">
						{quickLinkList && quickLinkList.length > 0 && quickLinkList.slice(0, 4).map((item, index) => {
							return (
								<div className="nav-item" onClick={() => { this.gotoUrl(item.redirectUrl + '?from=home') }} key={'nav' + index}>
									<img src={item.imgUrl} />
									<span>{item.name}</span>
								</div>
							)
						...
						<div className={hideHotCar ? 'hidden' : 'home-module hot-cars'}>
							...
							<Flex className="hot-cars-content">
								{hotCarList.map((item, index) => {
									return (
										<Flex.Item className={item.imgUrl ? "car-item" : 'invisible'} key={'hotcar' + index}>
					...
					<div className="home-module last-activity">
						...
						<div>
							{hotActivityList.map((item, index) => {
								return (
									<div className="card mb-30" key={'activity' + index} onClick={(e) => window.app.routerGoTo('/activityDetails?from=home&activityId=' + item.id)}>
										<div className="card-content">
											<img src={item.image} />
										</div>
							...
							<div className='pdc_footer'>
								<div className='btn_MoreComment' onClick={this.toUrl.bind(this, '/activity')}>查看更多</div>
							</div>
